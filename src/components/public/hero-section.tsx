...
import { NAVBAR_HEIGHT } from "@/lib/public-i18n";
...
export function HeroSection({
  ...
  return (
    <section
      className="relative flex min-h-screen flex-col items-center justify-center px-4 pb-20 sm:px-6 lg:px-8"
      style={{ paddingTop: NAVBAR_HEIGHT + 24 }}
    >
      <div className="relative mx-auto w-full max-w-4xl animate-fade-in text-center">
        {countriesLine && (
          <p className="mb-3 text-xs font-medium tracking-wide text-teal-200/90 sm:mb-4 sm:text-sm">
            {countriesLine}
          </p>
        )}
        <h1 className="mx-auto max-w-3xl text-2xl font-bold leading-tight sm:text-4xl md:text-5xl lg:text-6xl">
          {title}
        </h1>
        {subtitle && (
          <p className="mx-auto mt-4 max-w-2xl text-sm text-slate-300 sm:mt-6 sm:text-base md:text-lg">
            {subtitle}
          </p>
        )}
